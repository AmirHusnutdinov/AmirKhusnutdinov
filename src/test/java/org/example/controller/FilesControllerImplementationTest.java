...
class FilesControllerImplementationTest {
  ...
  void shouldFailToRenameFile() throws Exception {
    doThrow(FileNotFoundException.class).when(filesService)
        .renameFile(any(String.class), any(String.class));
    mockMvc.perform(get("/files/info/notFoundOldFileName/newFileName"))
        .andExpect(status().isNotFound());
  }

  @Test
  void shouldSuccessfullyReplaceFile() throws Exception {
    mockMvc.perform(get("/files/info/oldBucketName/fileName/newBucketName"))
  ...
  void shouldFailToReplaceFile() throws Exception {
    doThrow(FileNotFoundException.class).when(filesService)
        .replaceFile(any(String.class), any(String.class), any(String.class));
    mockMvc.perform(get("/files/info/notFoundOldBucketName/fileName/newBucketName"))
        .andExpect(status().isNotFound());
  }
  ...
  void shouldFailToRenameBucket() throws Exception {
    doThrow(BucketNotFoundException.class).when(filesService)
        .renameBucket(any(String.class), any(String.class));
    mockMvc.perform(get("/buckets/info/notFoundOldBucketName/newBucketName"))
        .andExpect(status().isNotFound());
  }
